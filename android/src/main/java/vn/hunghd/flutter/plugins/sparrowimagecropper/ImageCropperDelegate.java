package vn.hunghd.flutter.plugins.sparrowimagecropper;
...
public class ImageCropperDelegate implements PluginRegistry.ActivityResultListener {
	...
	if("png".equals(compressFormat)){
        outputFile = new File(outputDir, "sparrow_image_cropper_" + (new Date()).getTime() + ".png");
	} else {
		outputFile = new File(outputDir, "sparrow_image_cropper_" + (new Date()).getTime() + ".jpg");
	}
